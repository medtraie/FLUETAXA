...
import { FuelUsage, FuelSupply, Agent } from '../types';
...
export default function MovementsModule({ agent }: MovementsModuleProps) {
  ...
  useEffect(() => {
    ...
    }, (err) => handleFirestoreError(err, OperationType.GET, 'fuel_usages'));

    let unsubSupplies = () => {};
    // Only subscribe to supplies if user is likely admin (role check)
    // The security rules will still enforce this, but preventing the call avoids permission-denied errors in console
    if (agent?.role === 'admin') {
      const qSupplies = query(collection(db, 'fuel_supplies'), orderBy('date', 'desc'));
      unsubSupplies = onSnapshot(qSupplies, (snap) => {
        setSupplies(snap.docs.map(doc => ({ id: doc.id, ...doc.data() } as FuelSupply)));
      }, (err) => handleFirestoreError(err, OperationType.GET, 'fuel_supplies'));
    }
